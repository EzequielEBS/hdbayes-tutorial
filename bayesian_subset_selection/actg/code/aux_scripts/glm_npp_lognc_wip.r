...
glm.npp.lognc.wip = function(
  ...
  # )
  glm_npp_prior = 
    cmdstanr::cmdstan_model("bayesian_subset_selection/actg/code/aux_scripts/glm_npp_prior.stan")
  ...
  log_density = function(pars, data){
    ...
    if (wip == 1){
      prior_lp   = dcauchy(beta[1], location = 0, scale = 10, log = T) +
        sum( dcauchy(beta[2:p], location = 0, scale = 2.5, log = T) )
    } else{
