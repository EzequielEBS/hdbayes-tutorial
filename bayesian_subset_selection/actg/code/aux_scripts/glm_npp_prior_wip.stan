...
data {
  int<lower=0>                        K; // total number of datasets (excluding the current data)
  ...
  matrix[s,K]                         lognc; // the j-th column is the log nc for a0_lognc using the j-th historical datasets
...
parameters {
  ...
  vector<lower=logit(a0_lower),upper=logit(a0_upper)>[K] logit_a0s;
}
...
model {
  // prior on beta
  if (wip == 1) {
    target += cauchy_lpdf(beta[1]|0,10) + 
              cauchy_lpdf(beta[2:p]|0,2.5);
  }
  else {
  ...
  if ( dist <= 2 ) {
    for ( k in 1:K ) {
      ...
      target += a0s[k] * glm_lp(y[ start_idx[k]:end_idx[k] ],
  ...
  for ( k in 1:K ) {
      target += -pp_lognc(a0s[k], a0_lognc, lognc[, k]);
  }
}