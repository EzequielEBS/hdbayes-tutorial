...
Here we describe the analysis performed in the section 3.3.1 of the paper.
...
Then to compute useful quantities for all models, we define the following functions. The first one computes the prior probability of a model; the second one computes the log-normalizing constant for $a_0$; the third one computes posterior samples of beta for a given model; and the last one computes posterior model probabilities.
...
samples_models <- function(data,
  ...
  logp0_models <- parallel::parLapply(
    cl = cl,
    X = formulas,
    fun = logp0,
    family = family,
  ...
  logml_models <- parallel::parLapply(
    cl = cl,
    X = post_betam,
