...
#' Predict survival probability at t years for a single arm a, S_a(t), under a CurePWEPH model (with non-PSIPP priors).
#' 
#' Uses posterior draws from a CurePWEPH model fit to a single arm (treated or untreated). The treatment indicator
...
#' @param post.samples      posterior draws from a CurePWEPH model fit under various priors (other than PSIPP), with 
...
get.surv.prob.curepwe <- function(
  ...
  S = p_cured + (1 - p_cured) * S_noncured # [number of posterior draws x number of subjects in `data`]
...
#' Predict survival probability at t years for a single arm a, S_a(t), under a CurePWEPH model with PSIPP.
#' 
#' The PWEPH component (which is the model for the non-cured population) yields stratum-specific baseline hazards
#' and has no covariates.
#'
#' @param t                 time at which to evaluate survival probability.
#' @param post.samples      posterior draws from a CurePWEPH model fit under PSIPP, with an attribute called 
#'                          'data' which includes the list of variables specified in the data block of the 
#'                          Stan program.
#'
get.surv.prob.curepwe.psipp <- function(
  ...
  # Obtain posterior draws of cure fraction (p_cured) for each stratum
  ...
  # For each stratum k, S[, k] contains the subject-specific survival probability at time t for each posterior draw
  S   = sapply(1:K, function(k){
    p_cured = p_curedMat[, k]
    return(p_cured + (1 - p_cured) * S_noncured[, k])
  }) # [number of posterior draws x number of strata]
